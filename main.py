...
connector = sqlite3.connect('Patients.db')
#connector = sqlite3.connect('Books.csv')
#connector=pd.read_csv("Books.csv")
cursor = connector.cursor()

connector.execute(
'CREATE TABLE IF NOT EXISTS Patients (P_NAME TEXT, P_ID TEXT PRIMARY KEY NOT NULL, Doc_NAME TEXT, P_STATUS TEXT, P_MED TEXT)'
)

# Functions
def Doc_Name():
	Cid = sd.askstring('Doctor Name', 'What is the Doctor\'s Name?\t\t\t')

	if not Cid:
		mb.showerror('Can\'t keep Doctor\'s Name empty, it must have a value')
	else:
		return Cid
...
def display_records():
	...
	curr = connector.execute('SELECT * FROM Patients')
	data = curr.fetchall()
...
def add_record():
	...
	if surety:
		try:
			connector.execute(
			'INSERT INTO Patients (BK_NAME, BK_ID, AUTHOR_NAME, BK_STATUS, CARD_ID) VALUES (?, ?, ?, ?, ?)',
...
def update_record():
	def update():
		...
		cursor.execute('UPDATE Patients SET BK_NAME=?, BK_STATUS=?, AUTHOR_NAME=?, CARD_ID=? WHERE BK_ID=?',
...
def remove_record():
	...
	cursor.execute('DELETE FROM Patients WHERE BK_NAME=?', (selection[0], ))
	print(selection[0])
	connector.commit()
...
def change_availability():
	...
	if BK_status == 'Issued':
		...
		if surety:
			cursor.execute('UPDATE Patients SET bk_status=?, card_id=? WHERE bk_id=?', ('Available', 'N/A', BK_id))
			connector.commit()
		else: mb.showinfo(
			'Cannot be returned', 'The book status cannot be set to Available unless it has been returned')
	elif(BK_status=="Available"):
		cursor.execute('UPDATE Patients SET bk_status=?, card_id=? where bk_id=?', ('Issued', issuer_card(), BK_id))
		connector.commit()
	else:
		print("Invalid")
...
Label(root, text='Clinic Management System', font=("Noto Sans CJK TC", 15, 'bold'), bg=btn_hlb_bg, fg='White').pack(side=TOP, fill=X)
...
# Left Frame
Label(left_frame, text='Patient Name', bg=lf_bg, font=lbl_font).place(x=98, y=25)
Entry(left_frame, width=25, font=entry_font, text=bk_name).place(x=45, y=55)

Label(left_frame, text='Patient ID', bg=lf_bg, font=lbl_font).place(x=110, y=105)
bk_id_entry = Entry(left_frame, width=25, font=entry_font, text=bk_id)
bk_id_entry.place(x=45, y=135)

Label(left_frame, text='Doctor Name', bg=lf_bg, font=lbl_font).place(x=90, y=185)
Entry(left_frame, width=25, font=entry_font, text=author_name).place(x=45, y=215)

Label(left_frame, text='Status of the Patient', bg=lf_bg, font=lbl_font).place(x=75, y=265)
...
# Right Bottom Frame
Label(RB_frame, text='PATIENT INVENTORY', bg=rbf_bg, font=("Noto Sans CJK TC", 15, 'bold')).pack(side=TOP, fill=X)
